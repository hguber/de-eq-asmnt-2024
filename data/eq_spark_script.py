...
if __name__ == "__main__": 
    ...
    eq_d = f'{ds}.eq_daily'
    eq_w = f'{ds}.eq_weekly'
    eq_m = f'{ds}.eq_monthly'
    eq_f = f'{ds}.eq_dataset'
    ...
    df_final.coalesce(1).write.option("header", "true").csv(gs_bucket_stage + '/processed/final', mode='overwrite')
    print(f'entire df dataset successfully saved to {gs_bucket_stage}')
    df_week.coalesce(1).write.option("header", "true").csv(gs_bucket_stage + '/processed/weekly/', mode='overwrite')
    print(f'weekly df dataset successfully saved to {gs_bucket_stage}')
    df_monthly.coalesce(1).write.option("header", "true").csv(gs_bucket_stage + '/processed/monthly/', mode='overwrite')
    print(f'monthly df dataset successfully saved to {gs_bucket_stage}')
    df_daily.coalesce(1).write.option("header", "true").csv(gs_bucket_stage + '/processed/daily/', mode='overwrite')
    print(f'daily df dataset successfully saved to {gs_bucket_stage}')

    df_final.write.mode("overwrite").format('bigquery').option('table', eq_f).option("header", "true").save()
    print(f'final df dataset successfully loaded to eq_events.eq_dataset')
    df_week.write.mode("overwrite").format('bigquery').option('table', eq_w).option("header", "true").save()
    print(f'weekly df dataset successfully loaded to eq_events.eq_weekly')
    df_monthly.write.mode("overwrite").format('bigquery').option('table', eq_m).option("header", "true").save()
    print(f'monthly df dataset successfully loaded to eq_events.eq_monthly')
    df_daily.write.mode("overwrite").format('bigquery').option('table',eq_d).option("header", "true").save()
